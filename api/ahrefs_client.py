...
def fetch_ref_domains(url: str, api_key: str) -> dict:
    ...
    try:
        resp = httpx.get(
            f"{_BASE}/site-explorer/referring-domains",
            params={
                "target":   url,
                "mode":     "exact",
                "select":   "domain_rating,referring_domain",
                "limit":    "1000",
        ...
        data    = resp.json()
        domains = data.get("referring_domains") or data.get("data") or []
        total   = len(domains)
        dr10    = sum(1 for d in domains if (d.get("domain_rating") or 0) >= 10)
...
def fetch_organic_keywords(url: str, api_key: str, limit: int = 10) -> list:
    ...
    try:
        ...
        data = resp.json()
        kws  = data.get("organic_keywords") or data.get("data") or []
        result = []
        for k in kws:
